...
# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_timer():
    start_button.config(command=nothing)
...
def count_down(count):
    ...
    if count > 0:
        ...
        if reps % 2 == 0:
            CHECK_MARK += "✔"
            checkmarks.config(text=CHECK_MARK)


# ---------------------------- UI SETUP ------------------------------- #
# initialize window with tk class
window = Tk()
# configure window
window.title("Pomodoro")
window.config(padx=100, pady=50, background=YELLOW)
# initialize canvas with canvas class
...
checkmarks = Label(fg=GREEN, bg=YELLOW, font=(FONT_NAME, 10, "bold"))
